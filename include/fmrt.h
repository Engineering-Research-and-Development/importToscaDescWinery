 ...
 * - FMRTKO
 *   Result obtained when this is the first library call
 *   invoked by the caller, or when table definition was not
 *   completed (i.e. fmrtDefineKey() not invoked)
 * - FMRTIDNOTFOUND
 *   tableId is not defined
 ***********************************************************/
 ...
 * - FMRTKO
 *   Result obtained when this is the first library call
 *   invoked by the caller, when the specified file
 *   pointer is NULL or when table definition was not
 *   completed (i.e. fmrtDefineKey() not invoked).
 *   In those cases the last parameter provides 0.
 *   This result code is also obtained when an error is
 *   detected while reading input lines from the file
 *   (e.g. incomplete line); in such cases the last
 *   parameter reports the line where the error has
 *   been detected. Elements read from the file up to
 ...
 * - FMRTKO
 *   Result obtained when this is the first library call
 *   invoked by the caller, when the keyMin parameter is
 *   greater than keyMax or when table definition was not
 *   completed (i.e. fmrtDefineKey() not invoked)
 * - FMRTIDNOTFOUND
 *   tableId is not defined
 * - FMRTOUTOFMEMORY
...
/***********************************************************
 * fmrtDumpTable() - v.1.1.0
 * ---------------------------------------------------------
 * This library call is used to dump the content of the
 * given table into a specified file in binary format.
 * It takes the following parameters:
 * - tableId
 *   unique identifier of the table between 0 and 255. The
 *   table shall be defined first through fmrtDefineTable()
 * - filePtr
 *   pointer to a file opened by the caller in write mode.
 *   The output will be written on that file, overwriting
 *   previous content. If NULL an error is provided
 * Please note that the file shall be opened before calling
 * this function, otherwise a run-time error will occur.
 * Similarly, the function call does not close the output
 * file, which must be closed by the caller
 ...
 *   Result obtained when this is the first library call
 *   invoked by the caller, when the specified file
 *   pointer is NULL or the file cannot be written
 * - FMRTIDNOTFOUND
 *   tableId is not defined
 ***********************************************************/
fmrtResult fmrtDumpTable (fmrtId, FILE *);


/***********************************************************
 * fmrtReloadTable() - v.1.1.0
 * ---------------------------------------------------------
 * This library call is used to reload the content of a
 * given table from a binay file dumped through the call
 * fmrtDumptable()
 * It takes the following parameters:
 * - tableId
 *   unique identifier of the table between 0 and 255. The
 *   table shall be defined first through fmrtDefineTable()
 * - filePtr
 *   pointer to a file opened by the caller in read mode.
 * Please note that the file shall be opened before calling
 * this function, otherwise a run-time error will occur.
 * Similarly, the function call does not close the input
 * file, which must be closed by the caller.
 * The operation is destructive; if the input table is not
 * empty, then existing data are overwritten.
 * Please note that this routine does not check anything
 * about the stored data. The caller is responsible to grant
 * that stored data actually correspond to the table
 * structure that is going to be reloaded. If this is not
 * the case, the final outcome is unpredictable.
 * ---------------------------------------------------------
 * Possible Return Values:
 * - FMRTOK
 *   The table has been successfully reloaded
 * - FMRTKO
 *   Result obtained when this is the first library call
 *   invoked by the caller, when the specified file
 *   pointer is NULL, when the file cannot be read or
 *   when table definition was not completed (i.e.
 *   fmrtDefineKey() not invoked)
 * - FMRTIDNOTFOUND
 *   tableId is not defined
 * - FMRTOUTOFMEMORY
 ...
 *   to reload the dump
 ***********************************************************/
fmrtResult fmrtReloadTable (fmrtId, FILE *);
